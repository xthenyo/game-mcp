...
export async function collectAnswers() {
  ...
  console.log(chalk.dim('  Select additional tools to integrate (space to toggle, enter to confirm)'));
  console.log('');
  ...
  const integrations = await checkbox({
    ...
    choices: [
      {
        name: `${chalk.red('Sentry')} ${chalk.dim('— Error tracking & crash reporting')}`,
        value: 'sentry',
      },
      {
        name: `${chalk.yellow('Firebase')} ${chalk.dim('— Backend, auth, Firestore, hosting')}`,
        value: 'firebase',
      },
      {
        name: `${chalk.blue('Steam')} ${chalk.dim('— Steamworks API, player stats, achievements')}`,
        value: 'steam',
      },
      {
        name: `${chalk.magenta('REAPER')} ${chalk.dim('— Music & SFX production (DAW integration)')}`,
        value: 'reaper',
      },
      {
        name: `${chalk.green('ComfyUI')} ${chalk.dim('— Local Stable Diffusion image/video generation')}`,
        value: 'comfyui',
      },
    ],
  });
